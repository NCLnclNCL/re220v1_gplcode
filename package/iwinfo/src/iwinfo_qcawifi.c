...
int qcawifi_get_assoclist(const char *ifname, char *buf, int *len)
{
	...
	if( (tl = get80211priv(ifname, IEEE80211_IOCTL_STA_INFO, tmp, 64*1024-1)) > 0 )
	{
		cp = tmp;
		bl = 0;
...
int qcawifi_get_scanlist(const char *ifname, char *buf, int *len)
{
	...
	else if( !!qcawifi_isvap(ifname, NULL) )
	{
		...
		/* if scan error or scan null, scan again(first make sure athX device is up), by jxx */
		if (ret < 0 || *len == 0)
		{
			while (!iwinfo_ifisup(ifname) && total_delay < 8)
			{
				sleep(1);
				total_delay ++;
			}
