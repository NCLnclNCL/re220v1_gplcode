...
#ifdef CONFIG_BRIDGE_DELIVER_CONTROL
	br_db_fre_fdb_lookup((struct net_bridge_port **)&to, skb);
#endif
...
#if 1	
	/* added by humin at 14.11.17
       For broadcast Dhcp packet, it's data at skb will be changed when forwarding to RootAp.
       And the dhcp packet forward to rootap before to local. So I choose skb_copy.
    */		
	if (is_broadcast_ether_addr(eth_hdr(skb)->h_dest) && skb->protocol == htons(ETH_P_IP))
	{
...
#ifdef CONFIG_BRIDGE_DELIVER_CONTROL
	if (br_db_fre_should_deliver(p, skb))
	{
		return prev;
	}
#endif
