...
#include <linux/timer.h>	/* Add by yangnaijun, 25Jan2013 for bug 64546 */
#ifdef CONFIG_SYSCTL
...
/* Add by yangnaijun, 25Jan2013  for bug 64546
* Limit the number of replies to bellow arp packets per seconds:
* 1. IP conflict garp packets.- * When receive such arp packets(in arp_process), we start a timer. The timer 
