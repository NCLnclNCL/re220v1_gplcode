...
static void esw_link_status_changed(int port_no, void *dev_id)
{
...
#endif    
    if(reg_val & 0x1) {
	printk("ESW: Link Status Changed - Port%d Link UP, Register Value: %0x\n", port_no, reg_val);
	...
	/* reset switch fdb table to avoid 5min lan block; by huluyao */
	mii_mgr_write(0x1f, 0x80, 0x8002);
#endif
...
static irqreturn_t esw_interrupt(int irq, void *dev_id)
{
	...
	printk("Link Status Changed, Register Value: %0x\n", reg_int_val);
        mii_mgr_write(31, 0x700c, 0x1f); //ack switch link change
//added by humin
	mii_mgr_read(31, 0x700c, &reg_int_val);
	printk("Link Status Ack, Register Value: %0x\n", reg_int_val);
//added end
	spin_unlock_irqrestore(&(ei_local->page_lock), flags);
	return IRQ_HANDLED;
}
...
#if defined (CONFIG_TPLINK_GPIO_RE305)  || defined (CONFIG_TPLINK_GPIO_RE205V1) || defined (CONFIG_TPLINK_GPIO_RE205V2)
	val &= 0xf003ffff;
	sysRegWrite(RALINK_SYSCTL_BASE + 0x64, val); // set P0~P4 EPHY LED mode
#else
#if defined (CONFIG_TPLINK_GPIO_RE200V2) || defined (TP_PRODUCT_RE205V1_MT7628) || defined (TP_PRODUCT_RE205V2_MT7628)
    val &= 0xf003f3ff;
	sysRegWrite(RALINK_SYSCTL_BASE + 0x64, val); // set P0~P4 EPHY LED mode
    //do nothing
#elif defined (CONFIG_TPLINK_GPIO_RE200V3)
    val &= 0xf003ffff;
...
#if defined (CONFIG_RALINK_MT7621)
#if defined (CONFIG_RALINK_GPIO_IMPROVE_TPLINK)
#if defined (CONFIG_RALINK_GPIO_RE350)
#else
	//RGMII2=Normal mode
	*(volatile u_long *)(RALINK_SYSCTL_BASE + 0x60) &= ~(0x1 << 15);
	//GMAC2= RGMII mode
...
#endif
#endif 
	printk("Gpio Mode Value: %0x\n", (*(volatile u32 *)(RALINK_SYSCTL_BASE + 0x60)));
