...
#if !defined(CONFIG_RT2880_SQUASHFS_IN_FLASH)
static struct mtd_partition rt2880_partitions[] = {
	{
                name:           "ALL",
                size:           MTDPART_SIZ_FULL,
...
#else
#include <generated/mtd_partition.h>
#endif  /* CONFIG_RT2880_SQUASHFS_IN_FLASH */
...
int ramtd_read(struct mtd_info *mtd, loff_t from, size_t len,
 ...
 */
int ramtd_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	u32 addr,len;
 ...
 */
int ramtd_write(struct mtd_info *mtd, loff_t to, size_t len,
...
EXPORT_SYMBOL(ramtd_erase);
EXPORT_SYMBOL(ramtd_read);
EXPORT_SYMBOL(ramtd_write);
EXPORT_SYMBOL(flash);
