...
MODRET auth_log_pass(cmd_rec *cmd) {
  ...
  _LOG_WITH_ADDR(_LOG_LOGIN_SUCC);
#endif
    ...
    switch (auth_code) {
...
#ifdef CONFIG_UGLY_LOG
        _LOG_WITH_ADDR(_LOG_LOGIN_NO_USER);
#endif
        goto auth_failure;
...
#ifdef CONFIG_UGLY_LOG
        _LOG_WITH_ADDR(_LOG_LOGIN_BAD_PWD);
#endif
        goto auth_failure;
      ...
      case PR_AUTH_AGEPWD:
        pr_log_auth(PR_LOG_NOTICE, "USER %s (Login failed): Password expired.",
          user);
#ifdef CONFIG_UGLY_LOG
        _LOG_WITH_ADDR(_LOG_LOGIN_PWD_EXPIRED);
#endif
        goto auth_failure;
      ...
      case PR_AUTH_DISABLEDPWD:
        pr_log_auth(PR_LOG_NOTICE, "USER %s (Login failed): Account disabled.",
          user);
#ifdef CONFIG_UGLY_LOG
        _LOG_WITH_ADDR(_LOG_LOGIN_ACCOUNT_DISABLED);
#endif
        goto auth_failure;
...
MODRET auth_user(cmd_rec *cmd) {
  ...
  /* Check login name for DenyLoginName */
  c = find_config(main_server->conf, CONF_PARAM, "DenyLoginName", TRUE);
  while (c != NULL) {
      pr_signals_handle();

      if (strcmp(c->argv[0], user) == 0) {
          /* Deny current login name */
          pr_response_send(R_530, _("Login incorrect."));
          pr_session_end(0);
      }

      c = find_config_next(c, c->next, CONF_PARAM, "DenyLoginName", TRUE);
  }
...
/* usage: DenyLoginName login-name */
MODRET set_denyloginname(cmd_rec *cmd) {
  config_rec *c = NULL;
...
static conftable auth_conftab[] = {
  ...
  { "DenyLoginName",    set_denyloginname,      NULL },
  { NULL,			NULL,				NULL }
};
