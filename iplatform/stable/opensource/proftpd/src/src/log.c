...
{
    /* FIXME: need to keep this size consistent with log module, not a good idea. */
#define _LOG_BUF_MAX_SIZE (512)
    ...
    len = _mklog(proj_id, msg_id, pid, buf, _LOG_BUF_MAX_SIZE, args);
    va_end(args);
    if (len < 0)
    {
        return;
