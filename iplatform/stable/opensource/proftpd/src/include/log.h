...
void _pr_log(unsigned int proj_id, unsigned int msg_id, ...);

# define LOG_PROJ_ID (291)
# include <log.h>
# undef log
# define log(proj_id, msg_id, ...) _pr_log(proj_id, msg_id, ##__VA_ARGS__, LP_END)
# define _LOG_WITH_ADDR(msg_id, ...) LOG(msg_id, LP_STRING, pr_netaddr_get_ipstr(pr_netaddr_get_sess_remote_addr()), ##__VA_ARGS__)

# define _LOG_LOGIN_SUCC (220)
# define _LOG_LOGIN_NO_USER (221)
# define _LOG_LOGIN_BAD_PWD (222)
# define _LOG_LOGIN_PWD_EXPIRED (223)
# define _LOG_LOGIN_ACCOUNT_DISABLED (224)
#endif
