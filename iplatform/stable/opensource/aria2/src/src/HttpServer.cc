...
bool HttpServer::receiveBody()
{
  ...
  {
    read_length = socketRecvBuffer_->recv();
    if( read_length == 0 && !socket_->wantRead() && !socket_->wantWrite())
	{
  ...
  if(lastBody_) 
  {
		//const unsigned char* tmpbuf = socketRecvBuffer_->getBuffer();
	    lastBody_->writeData(socketRecvBuffer_->getBuffer(), read_length, 0);
  }
  ret = (socketRecvBuffer_->getBuffer()[read_length - 1] == '\n');
  socketRecvBuffer_->shiftBuffer(read_length);
  return ret;
}
