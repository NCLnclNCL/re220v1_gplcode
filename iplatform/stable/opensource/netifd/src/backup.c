...
static void switch_network_type(enum network_type type, struct interface *iface)
{
	...
	if (curr_backup_cfg.current_nw != type)
	{
		...
		{
			//PRINTF_ECHO("%s_%d: set default route: _%s_%s_", __FUNCTION__, __LINE__, wan_ifname, buf);
			system_exec_fmt("ip route replace default via %s dev %s", buf, wan_ifname);
		}
		else
		{
			//PRINTF_ECHO("%s_%d: set default route fail!!!! ip len is %d, wan_ifname is: _%s_", __FUNCTION__, __LINE__, strlen(buf), wan_ifname);
		}
	}
	else
	{
...
recv_step:
	...
	if (n >= 76)
	{
		...
		{
			//PRINTF_ECHO("%s_%d: drop pkg %d bytes!", __FUNCTION__, __LINE__, n);
		}
		return ret;
	}
...
detect_over(struct detect_fsm* fsm, unsigned* phase)
{
	...
	*phase = D_INITIAL;
	...
	{
		//PRINTF_ECHO("network type %d is online! switch to it.\n", curr_backup_cfg.detecting_nw);
		switch_network_type(curr_backup_cfg.detecting_nw, detect_if);
		//USBMODEM_DBG(LOG_NETWORK_ON, LP_UINT32, curr_backup_cfg.detecting_nw);
		curr_backup_cfg.detecting_nw = NETWORK_TYPE_NONE;
	}
	else
	{
		//PRINTF_ECHO("network type %d is offline!", curr_backup_cfg.detecting_nw);
		set_next_detect_if();
		if (curr_backup_cfg.detecting_nw == NETWORK_TYPE_NONE)
		{
			//PRINTF_ECHO("all network is offline! set network type 0 and do nothing.\n");
			curr_backup_cfg.current_nw = NETWORK_TYPE_NONE;
...
detect_timeout(struct uloop_timeout *timeout)
{
	...
	{
		//PRINTF_ECHO("!!!test network %d is connecting or disconnecting, set network type 0", res);
		curr_backup_cfg.current_nw = NETWORK_TYPE_NONE;
		goto go_next;
	}
