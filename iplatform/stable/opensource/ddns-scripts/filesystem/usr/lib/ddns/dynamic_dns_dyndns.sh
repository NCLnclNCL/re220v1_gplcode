...
load__all_config_options "ddns" "$1"
    ...
    if [ -n "$http_res" -a "$http_res" == "curl" ]; then
        ...
        case $http_code in
            ...
            404) 
                ...
                ddns_syslog $DDNS_LOG_WNG_RES_NOT_FOUND "dyndns" "${username}"
            ;;
            *) 
                dyndns_code=$DYNDNS_RETCODE_UNKNOWN
                ddns_syslog $DDNS_LOG_WNG_SVR_RESET "dyndns" "${username}" 
            ;;
        esac
    else
        ...
        ddns_syslog $DDNS_LOG_INF_RETCODE "dyndns" $result "${username}"
    fi
...
dyndns_set_state()
{
    ...
    if [ $code -eq $DYNDNS_RETCODE_GOOD -o $code -eq $DYNDNS_RETCODE_NOCHG ]; then
        ...
        ddns_syslog $DDNS_LOG_NTC_DOMAIN_FAIL "dyndns" "${username}"
    else
        uci_toggle_state ddns dyndns state $DYNDNS_STATE_STOP 
        ddns_syslog $DDNS_LOG_NTC_LOGIN_FAIL "dyndns" "${username}"
    fi
