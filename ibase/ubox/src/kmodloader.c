...
#include <glob.h>
#include <elf.h>
/* added by humin at 2016.07.13 for dir search*/
#include <dirent.h>
/* added end */
...
#define DEF_MOD_DIR	"/lib/modules/"
...
static int search_dir(char * path, char* name)
{
	 DIR *d; 
    struct dirent *file; 
    char   childpath[512];
    ...
    if(!(d = opendir(path)))
    {
        LOG("error opendir %s, error string: %s!!!\n", path, strerror(errno));
        return -1;
    }
    while((file = readdir(d)) != NULL)
    {
   		memset(childpath, 0, sizeof(childpath)); 
        //LOG("%s-%d file name: %s, isdir:%d\n", __FUNCTION__, __LINE__, file->d_name, file->d_type == DT_DIR);
       ...
       // LOG("%s-%d childpath : %s\n", __FUNCTION__, __LINE__, childpath);
        ...
        if(strcmp(get_module_name(file->d_name), name) == 0)
        {
			sprintf(mod_path, "%s", childpath);
			LOG("path:%s\n", childpath);  
			closedir(d);
         	return 0;
        }
...
static char* get_module_path_iplatform(char *name)
{
    LOG("%s-%d get name: %s\n", __FUNCTION__, __LINE__, name);
	memset(mod_path, 0, 512);
	if (!search_dir("/lib/modules", name))
	{
		return mod_path;
	}
	else
...
static char* get_module_path(char *name)
{
	...
	if (!stat(path, &s))
		return path;
    return get_module_path_iplatform(name);
//	return NULL;
}
...
static int scan_module_folder_iplatform(char * path)
{
	 DIR *d; 
    struct dirent *file;
    char   childpath[512];
    ...
    while((file = readdir(d)) != NULL)
    {
        ...
        if(file->d_type == DT_DIR)
        {
			 scan_module_folder_iplatform(childpath);
        }
        else
        {
			...
			if (glob(childpath, gl_flags, NULL, &gl) < 0)
			{
                LOG("%s-%d error glob %s, error string: %s!!!\n", __FUNCTION__, __LINE__,
                    path, strerror(errno));
                closedir(d);
				return -1;
			}
...
static int scan_module_folder(void)
{
	...
	globfree(&gl);
    scan_module_folder_iplatform("/lib/modules");
	return 0;
}
